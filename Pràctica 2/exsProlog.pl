...
intersection([], _, []).
intersection([X|Xs], L2, [X|L3]) :- member(X, L2), intersection(Xs, L2, L3).
intersection([X|Xs], L2, L3) :- \+ member(X, L2), intersection(Xs, L2, L3).
...
union([X|Xs], L2, [X|L3]) :- \+ member(X, L2), union(Xs, L2, L3).
...
inversa([], []).
inversa([X|Xs], F) :- append(L, [X], F), inversa(Xs,L).
...
fib(0, 0).
fib(1, 1).
fib(N, F) :- N1 is N-1, fib(N1, X), N2 is N-2, fib(N2, Y), F is X+Y.
...
dados(P, 0, L) :- P = 0, L = [].
dados(P, N, L) :- between(1, 6, X), N1 is N-1, P1 is P-X, P1 >= 0, dados(P1, N1, L1), L = [X|L1].
...
suma_la_resta(L) :- append(H, [X|Xs], L), suma(H, S), suma(Xs, R), X is S+R.
...
count(_, [], 0).
count(X, [X|Xs], C) :- count(X, Xs, C1), C is C1+1.
count(X, [Y|Xs], C) :- X \= Y, count(X, Xs, C).
card([]) :- true.
card([X|Xs]) :- count(X, [X|Xs], C), write([X,C]), delete(Xs, X, R), card(R).
...
% esta_ordenada([X|Xs]) :- append(Y, [Ys], [Y|Ys]), append([X], [Y|Ys], [X|Xs]),  X =< Y, esta_ordenada([Y|Ys]).
% esta_ordenada([_]) :- write('yes'), true.

esta_ordenada([_]) :- write('yes').
esta_ordenada([X,Y|Ys]) :- X =< Y, esta_ordenada([Y|Ys]).
esta_ordenada([X,Y|_]) :- X > Y, write('no').
...
palindroms(L) :- permutation(L, P), reverse(P, P), write(P).
...
gira(f(X,Y), f(Y,X)).
p(L,[S|P]) :- select(X,L,R), gira(X,S), p(R,P).
...
flatten([X|Xs], F) :- flatten(X, F1), flatten(Xs, F2), append(F1, F2, F).
...
mainn :-
    ...
    RSC1 is SC1/(SC1+SNC1), RNSC1 is NSC1/(NSC1+NSNC1),
    RSC1 > RNSC1,
    ...
    RSC2 is SC2/(SC2+SNC2), RNSC2 is NSC2/(NSC2+NSNC2),
    RSC2 > RNSC2,

    SC is SC1+SC2, SNC is SNC1+SNC2, NSC is NSC1+NSC2, NSNC is NSNC1+NSNC2,
    20 is SC+SNC+NSC+NSNC,
    RSC is SC/(SC+SNC), RNSC is NSC/(NSC+NSNC),
    RSC < RNSC, write([SC1,SNC1,NSC1,NSNC1,SC2,SNC2,NSC2,NSNC2]).
