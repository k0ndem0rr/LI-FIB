...
satVariable( ed(E,D,M)) :- event(E), day(D), moderator(M).
...
writeClauses :-  
    ...
    eachDayAtMostMaxEvents,
    eachModeratorAtMostMaxDays,
    eachEventPossibleDay,
    eachEventPossibleModerator,
    eachEventPossibleDayAndModerator,
    true,!.
...
eachDayAtMostMaxEvents :- day(D), maxEventsPerDay(Max), findall(ed(E,D), event(E), Lits), atMost(Max,Lits), fail.
eachDayAtMostMaxEvents.

eachModeratorAtMostMaxDays :- moderator(M), maxDaysPerModerator(Max), findall(ed(E,D), (event(E), day(D), em(E,M)), Lits), atMost(Max,Lits), fail.
eachModeratorAtMostMaxDays.

eachEventPossibleDay :- event(E), eventDays(E,Ds), findall(ed(E,D), member(D,Ds), Lits), exactly(1,Lits), fail.
eachEventPossibleDay.

eachEventPossibleModerator :- event(E), eventModerators(E,Ms), findall(em(E,M), member(M,Ms), Lits), exactly(1,Lits), fail.
eachEventPossibleModerator.

eachEventPossibleDayAndModerator :- event(E), eventDays(E,Ds), eventModerators(E,Ms), findall(ed(E,D,M), (member(D,Ds), member(M,Ms)), Lits), exactly(1,Lits), fail.
eachEventPossibleDayAndModerator.
