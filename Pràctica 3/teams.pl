...
incompatibleWorkers(W1,W2) :- workerScore(W1,S1), workerScore(W2,S2), maxScore(S), S1+S2 > S. %% Complete this!
...
writeClauses :-
        eachWorkerInOneTeam,
        eachTeamHasAtLeastMinWorkers,
        eachTeamHasAtMostMaxWorkers,
        avoidIncompatibleWorkers, 
    true,!.
writeClauses :- told, nl, write('writeClauses failed!'), nl,nl, halt.

eachWorkerInOneTeam :- worker(W), findall(wt(W,T), team(T), Lits), exactly(1, Lits), fail.
eachWorkerInOneTeam.

eachTeamHasAtLeastMinWorkers :- team(T), findall(wt(W,T), worker(W), Lits), minSize(S), atLeast(S, Lits), fail.
eachTeamHasAtLeastMinWorkers.

eachTeamHasAtMostMaxWorkers :- team(T), findall(wt(W,T), worker(W), Lits), maxSize(S), atMost(S, Lits), fail.
eachTeamHasAtMostMaxWorkers.

avoidIncompatibleWorkers :- incompatibleWorkers(W1,W2), team(T), writeOneClause([ -wt(W1,T), -wt(W2,T) ]), fail.
avoidIncompatibleWorkers.
