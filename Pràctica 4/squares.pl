...
%1: Variables i dominis:
    ...
    RowVars ins 1..Big, % in the worst case, the big square is filled with the smallest squares
    ColVars ins 1..Big, % in the worst case, the big square is filled with the smallest squares
%2: Constraints:
    insideBigSquare(Big, Sides, RowVars),
    insideBigSquare(Big, Sides, ColVars),
    nonoverlapping(N, Sides, RowVars, ColVars),
%3: Labeling:
    append(RowVars, ColVars, Vars), label(Vars),
    labeling([ff], Vars),
